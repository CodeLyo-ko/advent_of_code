...
int add_antinode(pos_t* antinodes, int* n_antinodes, int x, int y, int n, int m) {
    if (x < 0 || x >= n || y < 0 || y >= m) {
        return 0;
    }
...
void add_all_antinodes(pos_t* antinodes, int* n_antinodes, int x1, int y1, int x2, int y2, int n, int m) {
    pos_t v;
    v.x = x2 - x1;
    v.y = y2 - y1;
    ...
    while (add_antinode(antinodes, n_antinodes, x1 - (v_.x * i), y1 - (v_.y * i), n, m)) {
        i++;
    }
    ...
    for (int i = 0; i < n_antenna; i++) {
        for (int j = 0; j < antennas[i].n - 1; j++) {
            for (int k = j + 1; k < antennas[i].n; k++) {
                ...
                add_all_antinodes(antinodes, &n_antinodes, x1, y1, x2, y2, n, m);
            }
        }
    }
