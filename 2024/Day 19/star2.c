...
void cache(cache_t** cache_tab, char* towel, int n, int i, long long res) {
...
int in_cache(cache_t** cache_tab, char* towel, int n, int i, long long* res) {
...
long long is_possible(cache_t** cache_tab, char** designs, int n_design, int* k_design, char* towel, int n_towel, int i) {
    ...
    for (int n = 0; n < n_design; n++) {
        for (int k = 0; k < k_design[n]; k++) {
            ...
            if (k == k_design[n] - 1) {
                    res += is_possible(cache_tab, designs, n_design, k_design, towel, n_towel, i + k_design[n]);

            }
        }
    }

    cache(cache_tab, towel, n_towel, i, res);
    return res;
}
...
int main() {
    ...
    cache_t* cache_tab = NULL;
        ...
        towel[n_towel] = '\0';
       total += is_possible(&cache_tab, designs, n_design, k_design, towel, n_towel, 0);
    }
