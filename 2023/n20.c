...
int main()
{
    ...
    while (c != EOF)
    {
        modules[n_module].name = calloc(20, sizeof(char));
    ...
    int** states = malloc(5 * sizeof(int*)); //save sg lm dh db states
    for (int i = 0; i < 4; i++)
    {
        states[i] = malloc(50 * sizeof(int));
    }

    int* n_states = calloc(5, sizeof(int));

    while(nb_button < 10000)
    {
        nb_button++;
        ...
        while(Lsignals->num_elm > 0)
        {
            ...
            if (!strcmp(s->receiver, "jm"))
            {
                for (int i = 0; i < n_module; i++)
                {
                    if (!strcmp(modules[i].name, "sg") && modules[i].state && states[0][n_states[0] - 1] != nb_button)
                    {
                        states[0][n_states[0]++] = nb_button;
                    }
                    else if (!strcmp(modules[i].name, "lm") && modules[i].state && states[1][n_states[1] - 1] != nb_button)
                    {
                        states[1][n_states[1]++] = nb_button;
                    }
                    else if (!strcmp(modules[i].name, "dh") && modules[i].state && states[2][n_states[2] - 1] != nb_button)
                    {
                        states[2][n_states[2]++] = nb_button;
                    }
                    else if (!strcmp(modules[i].name, "db") && modules[i].state && states[3][n_states[3] - 1] != nb_button)
                    {
                        states[3][n_states[3]++] = nb_button;
                    }
                }
            }
