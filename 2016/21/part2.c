...
void move(char* password, int b, int a) {
...
int main() {
    ...
    //input
    sprintf(password, "fbgdceah");

    char** buff = malloc(MAX_ * MAX_ * sizeof(char));
    int n_buff = 0;
    size_t size;
    while(getchar() != EOF) {
        fseek(stdin, -1, SEEK_CUR);
        getline(&buff[n_buff], &size, stdin);
        n_buff++;
    }

    for (int i = n_buff - 1; i >=0; i--) {
        do_step(buff[i], password);
    }
